   ...
   $("#uploadphoto").change(function(){
  	 $("#myupload").ajaxSubmit({ 
  		...
  		success: function(data) {
			...
			if(data.status == 1){
				var src = data.url;  
				 $.post("/admin.php?s=/fly/pidata/y/3",{url:src},function(data){
						alert(data);
						history.go(0);
				});
			}else{
...
<!--上传图片插件end-->
	...
	<div class="cf">
	<div class="fl">
            <a class="btn" href="<?php echo U("fly/tedite");?>">新 增</a>
		...
		<table>
			...
			<tbody>
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($vo["threecode"]); ?></td>
						<td><?php echo ($vo["airport"]); ?></td>
						<td><?php echo ($vo["country"]); ?></td>
						
						<td>
							<a  title="编辑" href="<?php echo U('fly/tedite','id='.$vo["id"]);?>">编辑</a>
							<a class="confirm ajax-get" title="删除" href="<?php echo U('fly/tdel','id='.$vo["id"]);?>">删除</a>
						</td>
					</tr>
